...
# =====================================
outdir = '/eos/user/m/malberti/www/MTD/TOFHIR2X/MTDTB_CERN_May23/timeResolution_2E14_20_25um/'
...
outfile   = ROOT.TFile.Open(outdir+'/plots_timeResolution_2E14_20_25um_TBMay23.root','recreate')
...
for ds in data_structs:
    ...
    for ov in Vovs[ds.moduleLabel]:
        ...
        if (ov in  g_SR_vs_bar[ds.moduleLabel].keys()): 
            ...
            '''
            g_DCR_vs_DCRNpe_average[ds.moduleLabel].SetPoint( g_DCR_vs_DCRNpe_average[ds.moduleLabel].GetN(), x,  g_DCR_vs_bar[ds.moduleLabel][ov].GetMean(2))
            ...
            g_DCR_vs_DCRNpe_average_all.SetPoint( g_DCR_vs_DCRNpe_average_all.GetN(), x,  g_DCR_vs_bar[ds.moduleLabel][ov].GetMean(2))
            ...
            g_DCRNpe_vs_DCR_average[ds.moduleLabel].SetPoint( g_DCRNpe_vs_DCR_average[ds.moduleLabel].GetN(), dcr,  g_DCR_vs_bar[ds.moduleLabel][ov].GetMean(2))
            g_DCRNpe_vs_DCR_average[ds.moduleLabel].SetPointError( g_DCRNpe_vs_DCR_average[ds.moduleLabel].GetN()-1, 0.5*(x_up-x_down),  g_DCR_vs_bar[ds.moduleLabel][ov].GetRMS(2))
            g_DCRNpe_vs_DCR_average_all.SetPoint( g_DCRNpe_vs_DCR_average_all.GetN(), dcr,  g_DCR_vs_bar[ds.moduleLabel][ov].GetMean(2))
            g_DCRNpe_vs_DCRN_average_all.SetPointError( g_DCRNpe_vs_DCR_average_all.GetN()-1, 0.5*(x_up-x_down),  g_DCR_vs_bar[ds.moduleLabel][ov].GetRMS(2))
            '''
...
for ds in data_structs:
    ...
    leg[ds.moduleLabel] = ROOT.TLegend(0.65,0.70,0.89,0.89)
    ...
    for i,bar in enumerate(bars[ds.moduleLabel]):
        ...
        xmin = 0.0
        xmax = 2.0
        hdummy1[ds.moduleLabel][bar] = ROOT.TH2F('hdummy1_%s_%d'%(ds.moduleLabel,bar),'',100,xmin,xmax,180,0,180)
        hdummy1[ds.moduleLabel][bar].GetXaxis().SetTitle('V_{OV}^{eff} [V]')
        ...
        if (i==0):
            ...
            #leg[ds.moduleLabel].AddEntry(g_Tot_vs_Vov[ds.moduleLabel][bar], 'stoch (+) noise', 'PL')
        ...
        hdummy1[ds.moduleLabel][bar].Delete()
        #c1[ds.moduleLabel][bar].Delete()
...
c2 =  ROOT.TCanvas('c_timeResolutionDCRNpe_vs_DCR_average','c_timeResolutionDCRNpe_vs_DCR_average',600,600)
c2.SetGridx()
c2.SetGridy()
c2.cd()    
hdummy2 = ROOT.TH2F('hdummy2_%d'%(bar),'',100,0,50,100,0,150)
hdummy2.GetXaxis().SetTitle('DCR[GHz]')
hdummy2.GetYaxis().SetTitle('#times #sigma_{t}^{DCR}/(Npe/6000) [ps]')
hdummy2.Draw()
...
for bar in range(0,16):
    ...
    for ds in data_structs:
        ...
        if (i==0):
            leg2.AddEntry(g_SR_vs_Vov[ds.moduleLabel][bar], '%s'%ds.moduleLabel, 'PL')
...
for ds in data_structs:
    for ov in Vovs[ds.moduleLabel]:
        ...
        ovEff = getVovEffDCR(data, ds.lyso, (ds.sipm+'_T%dC'%ds.temperature), ('%.02f'%ov))[0]        
        c5[ov] = ROOT.TCanvas('c_slewRate_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),'c_slewRate_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),600,600)
        c5[ov].SetGridy()
        c5[ov].cd()
        #hdummy5[ov] = ROOT.TH2F('hdummy5_%d'%(ov),'',100,-0.5,15.5,100,0,15)
        hdummy5[ov] = ROOT.TH2F('hdummy5_%s_%.2f'%(ds.moduleLabel,ov),'',100,-0.5,15.5,100,0,35)
        ...
        c6[ov] = ROOT.TCanvas('c_bestTh_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),'c_bestTh_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),600,600)
        c6[ov].SetGridy()
        c6[ov].cd()
        hdummy6[ov] = ROOT.TH2F('hdummy6_%s_%.2f'%(ds.moduleLabel,ov),'',100,-0.5,15.5,100,0,20)
        ...
        c7[ov] = ROOT.TCanvas('c_noise_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),'c_noise_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),600,600)
        c7[ov].SetGridy()
        c7[ov].cd()
        hdummy7[ov] = ROOT.TH2F('hdummy7_%s_%.2f'%(ds.moduleLabel,ov),'',100,-0.5,15.5,100,0,80)
        ...
        c8[ov] = ROOT.TCanvas('c_stoch_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),'c_stoch_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),600,600)
        c8[ov].SetGridy()
        c8[ov].cd()
        hdummy8[ov] = ROOT.TH2F('hdummy8_%s_%.2f'%(ds.moduleLabel,ov),'',100,-0.5,15.5,100,0,80)
        ...
        c9[ov] = ROOT.TCanvas('c_dcr_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),'c_dcr_vs_bar_%s_Vov%.2f'%(ds.moduleLabel,ovEff),600,600)
        c9[ov].SetGridy()
        c9[ov].cd()
        hdummy9[ov] = ROOT.TH2F('hdummy9_%s_%.2f'%(ds.moduleLabel,ov),'',100,-0.5,15.5,100,0,140)
