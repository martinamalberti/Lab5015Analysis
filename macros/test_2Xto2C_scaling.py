...
fnames = { '2X' : '/eos/user/m/malberti/www/MTD/TOFHIR2X/MTDTB_CERN_May23/timeResolution_2E14_20um_25um_T2_v2/plots_timeResolution_2E14_20um_25um_T2_TBMay23_TOFHIR2X.root',
           '2C' : '/eos/user/m/malberti/www/MTD/TOFHIR2C/MTDTB_CERN_May23/timeResolution_2E14_20um_25um_T2/plots_timeResolution_2E14_20um_25um_T2_TBMay23_TOFHIR2C.root'
}
    ...
    for label in labels:
        ...
        if (tofhir == '2X'):
            for i in range(0, gData[tofhir, label].GetN()):
                ...
                s_tot_up = math.sqrt(s_noise_scaled*s_noise_scaled*1.05*1.05 + s_stoch*s_stoch + s_dcr*s_dcr) # assume 5% error on noise estimation (... to be checked)
                s_tot_down = math.sqrt(s_noise_scaled*s_noise_scaled*0.95*0.95 + s_stoch*s_stoch + s_dcr*s_dcr)
...
for label in labels:
    ...
    for tofhir in ['2X','2C']:
        ...
        if (tofhir == '2X'):
            gData_scaled[tofhir,label].SetLineColor(plotAttrs[tofhir][1]+2)
            gData_scaled[tofhir,label].SetLineStyle(2)
            gData_scaled[tofhir,label].SetLineWidth(2)
            gData_scaled[tofhir,label].SetFillStyle(1)
            gData_scaled[tofhir,label].SetFillColorAlpha(plotAttrs[tofhir][1]+2, 0.2)
            
            leg.AddEntry(gData_scaled[tofhir,label], '%s - scaled'%plotAttrs[tofhir][2],'FL')
            gData_scaled[tofhir,label].Draw('E3lsame')
    gData_scaled['2X',label].Draw('lsame')
    leg.Draw()
