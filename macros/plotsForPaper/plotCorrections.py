...
p1PhaseCorr_0 = f1.Get('p1_deltaT_totRatioCorr_vs_t1fineMean_bar07L-R_Vov%.2f_th%02d_energyBin01'%(ov,th))
h2PhaseCorr_0 = f1.Get('h2_deltaT_totRatioCorr_vs_t1fineMean_bar07L-R_Vov%.2f_th%02d_energyBin01'%(ov,th))
...
hPad = ROOT.gPad.DrawFrame(0.,0.1,1000.0,hMIPpeak.GetMaximum()*1.1)
hPad.SetTitle(';energy [ADC]; entries')
#hPad.Draw()
ROOT.gPad.SetTicks(1)
hMIPpeak.Draw('same')
#hMIPpeak.GetXaxis().SetNdivisions(505)
fun = hMIPpeak.GetFunction('f_landau_bar07L-R_Vov%.2f_vth1_%02d'%(ov,th))
fun.SetLineColor(1)
fun.Draw('same')
...
tl.DrawLatex(0.65,0.82,'#splitline{V_{OV} = %.2f V}{th. = %d DAC}'%(ov,th))
cms_logo = draw_logo()
cms_logo.Draw()
...
# Energy ratio corr
xmin = p1EnergyCorr_0.GetMean(1)-0.2
xmax = p1EnergyCorr_0.GetMean(1)+0.2
