...
for mod in modules:
   ...
   leg = ROOT.TLegend(0.20, 0.68, 0.50, 0.88)
   leg.SetBorderSize(0)
   leg.SetFillStyle(0)
   if (len(Vovs[mod])>4):
      leg.SetNColumns(2);
   ...
   h[mod] = ROOT. TH1F('h_%s'%mod,'h_%s'%mod, 50, -0.75,0.75 )
   h[mod].SetLineColor(1)
   h[mod].SetLineWidth(1)
   ...
   hPad2 = ROOT.TPad("subpad","",0.6, 0.6, 0.92, 0.92)
   hPad2.SetBottomMargin(0.2)
   hPad2.Draw()
   hPad2.cd()
   h[mod].Draw()
   h[mod].GetXaxis().SetNdivisions(5)
   h[mod].GetYaxis().SetNdivisions(5)
   h[mod].GetXaxis().SetLabelSize(0.090)
   h[mod].GetYaxis().SetLabelSize(0.090)
   h[mod].GetXaxis().SetTitleSize(0.090)
   h[mod].GetXaxis().SetTitle('(#sigma_{t} - <#sigma_{t}>)/<#sigma_{t}>')
   latex2 = ROOT.TLatex(0.65,0.75,'RMS = %.0f %%'%(h[mod].GetRMS()*100))
   latex2.SetNDC()
   latex2.SetTextSize(0.080)
   latex2.SetTextFont(42)
   latex2.Draw()
   
   
   c.cd()
   latex = ROOT.TLatex(0.20,0.22,'%s'%(labels[mod]))
   latex.SetNDC()
